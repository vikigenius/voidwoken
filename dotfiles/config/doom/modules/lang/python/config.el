...
(use-package! rye-mode
  :when (modulep! +rye)
  :after python
  :config
  (when (executable-find "rye")
    (add-to-list 'exec-path (expand-file-name "shims" (or (getenv "RYE_HOME") "~/.rye")))))
...
(eval-when! (and (modulep! +lsp)
  ...
  (use-package! lsp-python-ms
    :unless (or (modulep! +pyright) (modulep! +basedpyright))
    :after lsp-mode
    :preface
    (after! python
